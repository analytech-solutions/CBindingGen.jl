...
function generate(io::IO, cvt::Converted, indent::Int; relto::String = ".", comments::Bool = true, exports::Bool = true)
	...
	if comments
		for (name, comment) in cvt.comments
			...
			comment = string(comment, expr = (startswith(cvt.expr, "𝐣𝐥.@cextern") ? cvt.expr : ""), relto = relto)
			for escape in COMMENT_ESCAPES
				comment = replace(comment, escape)
			end
			comment = """
			𝐣𝐥.@doc \"\"\"
			$(rstrip(comment, '\n'))
